...
void intraReading(char* reconstructedFrame, int* modeUpBlock, int* modeLeftBlock, int blockSize, int y, int x)
{
    ...
    if (y >= blockSize)
    {
        for (int i = 0; i < blockSize; ++i)
        {
            modeY = i * blockSize;
            tempY = y - blockSize + i;
            if (tempY >= HEIGHT) tempY = HEIGHT - 1;
            tempY *= LENGTH;
            for (int j = 0; j < blockSize; ++j)
            {
                tempX = x + j;
                if (tempX >= LENGTH) tempX = LENGTH - 1;
...
int splitBlockPadding(char* tempFrame, char* previousFrame, char* reconstructedFrame, int* frameCoef, int* modeMatrix,
    int* qMatrix, int blockSize, int y, int x, int frameN, double frameVAR, bool haveToCalculateVAR)
{
    ...
    else
    {
        ...
        QP = qMatrix[(y / blockSize) * wBlock + (x / blockSize)];
    }
    ...
    {
        if (residualBlock[i] != 0) dataCount++;
    }
...
int split(char* startFrame, char* previousFrame, char* reconstructedFrame,
    int* frameCoef, int* modeMatrix, int* qMatrix, int blockSize, int frameN, bool haveToCalculateVAR, double frameVAR)
{
    int dataCount = 0;
    for (int i = 0; i < HEIGHT; i += blockSize)
    {
        for (int j = 0; j < LENGTH; j += blockSize)
        {
            dataCount += splitBlockPadding(startFrame, previousFrame, reconstructedFrame,
                frameCoef, modeMatrix, qMatrix, blockSize, i, j, frameN, frameVAR, haveToCalculateVAR);
        }
    }
...
void encoder(int blockSize, const char* in_string)
{
    ...
    while (frameCount < 10)
    {
        ...
        if (frameCount > 0)
        {
            //compare 2 frames and if difference is big, then it`s a scene changing, so recalculate variance of frame and blocks
            double sum = 0;
            for (int i = 0; i < pixels_on_video; ++i)
            {
                int d = (unsigned char)greyscaleFrame[i] - (unsigned char)previousFrame[i];
                double a = (unsigned char)greyscaleFrame[i];
                d *= d;
                sum += d / a;
            }
            ...
            if (sum >= 1)
            {
                haveToCalculateVAR = true;
                frameVAR = variance(greyscaleFrame, pixels_on_video);
            }
            else haveToCalculateVAR = false;
        }
        else frameVAR = variance(greyscaleFrame, pixels_on_video);

        dataCount += split(greyscaleFrame, previousFrame, reconstructedFrame,
            frameCoef, modeMatrix, qMatrix, blockSize, frameCount, haveToCalculateVAR, frameVAR);
    ...
    //dataCount /= pixels_on_video;
    //cout << "Data Compression: " << 1.0 - dataCount / frameCount << endl;
...
void secondEncoder(int blockSize, const char* in_string)
{
    const char* mode_string = "mode_file.dat";
    const char* coef_string = "coef_file.dat";
    const char* q_string    = "q_file.dat";
    ...
    while (frameCount < 10)
    {
        ...
        dataCount += split(greyscaleFrame, previousFrame, reconstructedFrame,
            frameCoef, modeMatrix, qMatrix, blockSize, frameCount, false, 0);
